...
public: /* Draw Primitives */
    ...
    void fill_triangle(const std::int32_t x1, const std::int32_t y1,
        ...
        const auto [x_top, y_top] = vertices;
        ...
        const float new_slope_right = static_cast<float>(x_bot - x_mid) / (y_bot -
            y_mid);
        for (std::int32_t y = y_mid + 1; y <= y_bot; ++y) {
            const auto x_start = static_cast<std::int32_t>(x_mid + (y - y_mid) *
                slope_left);
            const auto x_end = static_cast<std::int32_t>(x_top + (y - y_top) *
                new_slope_right);
            scanlines[y - y_top] = {x_start, x_end};
    ...
    void draw_circle(const std::int32_t center_x, const std::int32_t center_y, const
        std::int32_t radius,
        const Pixel color) {
